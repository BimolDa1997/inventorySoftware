...
if(isset($_POST['save'])){
  ...
  $insert =$conn->query("insert into greeting_info(`title`,`description`,`priority`,`entry_by`, `status`) 
  
  Values('".$title."','".$description."', '".$priority."','". $entry_by."', '".$status."')");
  if($insert==true)
  {
...
//photo
if($_FILES['pic']['name']!=''){
$target_dir = "admin/files/greeting/pic/";
...
$pic_update = $conn->query("update greeting_info set pic='".$target_dir.$file_name."' where id='".$insert_id."'");
}
}
//photo end
...
if(isset($_POST['update'])){
  ...
  if($update_id>0){
  $update = $conn->query("update greeting_info set title='".$title."',description='".$description."',status='".$status."' where id='".$update_id."'");
  }
 ...
 /* echo '<script>window.location.href = "customer_setup.php";</script>';*/
  ...
  $msg =  '<span style="color:red; font-weight:bold; font-size:18px;">Plesae Try Again!</span>';
	...
	if(isset($_GET['del'])){
		...
		$image = $conn->query("select pic from greeting_info where id='".$del_id."'");
		$image_data = $image->fetch_object();
		$image_path = $image_data->pic;
		unlink($image_path);
		$delete = $conn->query("delete from greeting_info where id='".$del_id."'");
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title">Greeting Card <?php  echo $msg;?></h4>
								...
								<div class="card-body">
														...
														<div class="row">
															<div class="col-sm-12">
																<div class="form-group form-group-default">
																	<label>Link <span style="color:#FF0000; font-size:15px;">*</span></label>
																	<input type="text" name="title"  id="title"  class="form-control" placeholder="Link" required>
																</div>
															</div>
															<div class="col-sm-12">
																<div class="form-group form-group-default">
									...
									<div class="table-responsive">
										<table id="add-row" class="display table table-striped table-hover" >
											...
											<tbody>
											<?php
											  $select =$conn->query("select * from greeting_info where 1 ");
											  $sl=1;
