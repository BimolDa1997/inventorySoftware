...
if(isset($_POST['save'])){
  ...
  $insert =$conn->query("insert into customer_info(`customer_name`,`business_name`,`propritor_name`,`mobile_no`,`customer_address`,`ledger_id`,`credit_limit`,`entry_by`,`customer_type`,`previous_due`,`previous_advance`, `group_for`,opening_date) 
  Values('".$business_name."','".$business_name."','".$propritor_name."','".$mobile_no."','".$customer_address."','".$ledger_id."','".$credit_limit."','". $entry_by."','Registered','".$previous_due."','".$previous_advance."', '".$group_for."', '".$opening_date."')");
  if($insert==true)
  {
...
$tr_from = 'Opening Due';
$cr_ledger = 1000002;
$jv_date=$opening_date;
//$jv_date = date('Y-m-d',strtotime($opening_date));
$status = 'Paid';
$entry_by = '';
...
if(isset($_POST['update'])){
  ...
  if($update_id>0){
  $update = $conn->query("update customer_info set customer_name='".$business_name."', business_name='".$business_name."',propritor_name='".$propritor_name."',mobile_no='".$mobile_no."',customer_address='".$customer_address."',credit_limit='".$credit_limit."',entry_by='".$entry_by."',previous_due='".$previous_due."',previous_advance='".$previous_advance."',group_for='".$group_for."' ,opening_date='".$opening_date."' where id='".$update_id."'");
  }
  ...
  $found_opening = find_a_field('journal','jv_no','ledger_id="'.$ledger.'" and tr_from="Opening Due"');
   $found_advance = find_a_field('journal','jv_no','ledger_id="'.$ledger.'" and tr_from="Opening Advance"');
  
  if($ledger>0){
    $del = $conn->query("delete from journal where jv_no='".$found_opening."' and tr_from='Opening Due'");
	 $del = $conn->query("delete from journal where jv_no='".$found_advance."' and tr_from='Opening Advance'");
	$max_jv = find_a_field('journal','max(jv_no)','1');
   ...
   // $jv_date = date('Y-m-d',strtotime($opening_date));
	...
	$rcv_cr1=$previous_due;	
	if($previous_due>0){
	$tr_from = 'Opening Due';

$rcv_dr1 = "INSERT INTO `journal` (`jv_no`, `jv_date`, `ledger_id`,`dr_amt`,`cr_amt`, `tr_from`, `tr_no`,`group_for`,`status`,`entry_by`) VALUES ('".$max_jv."','".$jv_date."','".$cr_ledger."',0,'".$previous_due."','".$tr_from."','".$insert_id."','".$group_for."','".$status."','".$entry_by."')";
$journal_insert_dr1 =$conn->query($rcv_dr1);

$rcv_cr1 = "INSERT INTO `journal` (`jv_no`, `jv_date`, `ledger_id`,`dr_amt`,`cr_amt`, `tr_from`, `tr_no`,`group_for`,`status`,`entry_by`) VALUES ('".$max_jv."','".$jv_date."','".$ledger."','".$previous_due."',0,'".$tr_from."','".$insert_id."','".$group_for."','".$status."','".$entry_by."')";
...
*/
 ...
 if(isset($_POST['file_upload'])){
		...
		$filename=$_FILES["bulk_file"]["tmp_name"];
		...
		$customer = trim($getData[0]);
		$address = trim($getData[1]);
		$mobile_1 = trim($getData[2]);
		$mobile_2 = trim($getData[3]);
		$email = trim($getData[4]);
		$address = str_replace( array( '\'', '"',"'" , ';', '<', '>' ), ' ', $address);
		
		
			 $check = find_a_field('customer_info','customer_name','customer_name="'.$customer.'"');
			 if($check==''){
			 echo $customer_sql = "insert into customer_info(`customer_name`,`mobile_no`,`mobile_no_2`,`email`,`customer_address`,`entry_at`,`entry_by`) value('".$customer."','".$mobile_1."','".$mobile_2."','".$email."','".$address."','".date('Y-m-d H:i:s')."','".$_SESSION['user_id']."')";
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
                                        ...
                                        <button type="submit" name="file_upload" id="file_upload" class="btn btn-primary btn-round ml-auto"><i class="fa fa-upload" style="font-size:30px;"><input type="file" name="bulk_file" id="bulk_file" class="btn btn-primary btn-round ml-auto" /></i></button>
                                        <a href="../files/customer/customer_info.csv" download><i class="fa fa-download" style="font-size:30px;"></i></a>
                                        </form>
														...
														<div class="row">
															<div class="col-sm-12">
																<div class="form-group form-group-default">
																	<label>Customer Name <span style="color:#FF0000; font-size:15px;">*</span></label>
															...
															<div class="col-md-6 pr-0">
																<div class="form-group form-group-default">
																	...
																	<input name="mobile_no" minlength="11" id="mobile_no" type="text" class="form-control" placeholder="Mobile No." required>
																</div>
															</div>
															<!-- <div class="col-md-6">
																<div class="form-group form-group-default">
															...
															<div class="col-md-6">
																<div class="form-group form-group-default">
																	<label>Opening Date <span style="color:#FF0000; font-size:15px;">*</span></label>
																	<input name="opening_date" id="opening_date" type="date" class="form-control" placeholder="Opening Date." required>
																</div>
														     </div> -->
													...
													<p class="small">Make Sure Fillup All Field</p>
														...
														<div class="row">
															...
															<div class="col-md-6 pr-0">
																<div class="form-group form-group-default">
																	<label>Mobile No. <span style="color:#FF0000; font-size:15px;">*</span></label>
																	<input name="mobile_no1" minlength="11" id="mobile_no1" type="text" value="<?=$c_data['mobile_no']?>" class="form-control" placeholder="Mobile No." required>
																</div>
															</div>
															<!-- <div class="col-md-6">
																<div class="form-group form-group-default">
															...
															<div class="col-md-6">
																<div class="form-group form-group-default">
																	<label>Opening Date. <span style="color:#FF0000; font-size:15px;">*</span></label>
																	<input name="opening_date" id="opening_date" type="date" value="<?=$c_data['opening_date']?>" class="form-control" placeholder="Opening Date." required>
																</div>
															</div>
														</div> -->
